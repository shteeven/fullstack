...
<<<<<<< HEAD
	flash, make_response
=======
    flash, make_response, exceptions
>>>>>>> 52f249d30d6685ce01437e3af38f282cb6a47ffe
...
# Show all categories; public or authed page rendered depending on user login
@app.route('/')
@app.route('/category/')
def showCategories():
    categories = session.query(Category).order_by(asc(Category.name)).all()
...
=======
    ...
    if request.method == 'POST':
        if request.form['name']:
            editedCategory.name = request.form['name']
            editCategory.img_url = request.form['img_url'],
            flash('Category Successfully Edited %s' % editedCategory.name)
            return redirect(url_for('showItems', category_id=category_id))
        else:
            flash("Please ensure there is a name for the category.")
...
# Show a category item; public or authed page rendered depending on user login
@app.route('/category/<int:category_id>/')
@app.route('/category/<int:category_id>/item/')
def showItems(category_id):
    try:
...
=======
    ...
    category = session.query(Category).filter_by(id=category_id).one()
    if login_session['user_id'] != category.user_id:
        return "<script>function myFunction() {alert('You are not authorized to add item items to this category. Please create your own category in order to add items.');}</script><body onload='myFunction()''>"
    if request.method == 'POST':
        if request.form['name']:
            ...
            flash('Please ensure that the item has a name.')
    else:
...
def editItem(category_id, item_id):
    ...
    category = session.query(Category).filter_by(id=category_id).one()
    if login_session['user_id'] != category.user_id:
        return "<script>function myFunction() {alert('You are not authorized to edit item items to this category. Please create your own category in order to edit items.');}</script><body onload='myFunction()''>"
    if request.method == 'POST':
        if request.form['name']:
        ...
        return render_template('edititem.html', category_id=category_id,
                               item_id=item_id, item=editedItem, logged='true')
>>>>>>> 52f249d30d6685ce01437e3af38f282cb6a47ffe
